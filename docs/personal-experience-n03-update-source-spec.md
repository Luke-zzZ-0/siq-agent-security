...
批次：n03-update-source-config-20260913（N03 第 1 步 / N03-A 前半：来源引用 + 调度元数据存储）；n03-update-scheduler-20260913（N03 第 2 步：守护进程调度器）；n03-update-source-http-20260914（N03 第 3 步：HTTP 端点 + Web 面板接线 + 契约样例）
状态：来源存储、调度器、HTTP、Web 面板及 daemon 周期任务已进入本批独立验收；完整原生更新旅程仍待验收。
...
5. 用户控制：`enable=false` 关闭自动检查（文件保留以记忆最后状态）；来源变更必须重新显式保存，重新保存重置检查状态。
...
- Web 数据层：`readSkillUpdateSource` / `saveSkillUpdateSource` 对响应跑校验器（精确键集、install_id 必须等于路径 id、displayURL 无 query/fragment/userinfo），不合规抛 502 `skill_install_incompatible_response`。调用方 zip 链接仅存 React state，成功保存后清空；不写 Web Storage、不进审计、不出现在 URL。git 启停永远携带空 URL。

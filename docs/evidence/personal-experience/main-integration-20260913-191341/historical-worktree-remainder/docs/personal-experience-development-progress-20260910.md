...
| UX-009 | doing / Hermes installation UI verified in isolated profile | 本地目录/ZIP、HTTPS ZIP、Git 来源（M131：硬编码 git CLI、钩子禁用、提交钉扎）、固定副本审阅、来源绑定批准、安装预览、明确安装确认与失败恢复已接通；隔离 Linux Chromium 和目标文件操作验证通过。安装后实例权限准备、原生清单识别与实例范围的 CLI 保护已验证；浏览器文件选择、平台安装入口拦截、可信 Skill 归属与其他平台仍待完成 |
| UX-010 | doing / update UI verified in isolated Linux browser | 记录、内容检查、移除与恢复 UI/API 已验证；更新比较、独立副本准备、明确切换和中断恢复核心/API 已实现。差异/确认/恢复 UI 已在隔离 Linux 浏览器验证。新版检查（M132：git/zip 上游只读重取快照、定位摘要绑定、200 项截断、零状态写入）已实现并测试；原生更新验收与通用旧状态写入拒绝仍待完成 |
